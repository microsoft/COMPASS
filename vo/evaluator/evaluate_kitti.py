...
def evaluate(gt, data, kittitype=True):
    ...
    errors = calculate_sequence_error(gt, data, lens)
    rot,tra = calculate_ave_errors(errors, lens)
    return np.mean(rot), np.mean(tra) # errors[:,1].mean() * 180./np.pi, errors[:,2].mean() #
...
def  main():
    ...
    predict_pose_data = np.loadtxt(sys.argv[2])
    gt_traj = traj2traj(ground_truth_data)
    est_traj = traj2traj(predict_pose_data)
    errors = calculate_sequence_error(gt_traj,est_traj)

    errors = np.array(errors)
    bb=np.loadtxt('/home/wenshan/Downloads/devkit_odometry/devkit/cpp/results/errors/00.txt')
